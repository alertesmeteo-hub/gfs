(function () {
    ...
    function fetchText(url) {
        ...
        return fetch(url, { cache: 'no-cache' }).then(read).catch(function (error) {
            // Même commit, même fichier : secours si le CDN raw renvoie 400.
            var match = url.match(/^https:\/\/raw\.githubusercontent\.com\/([^/]+)\/([^/]+)\/([a-f0-9]{40})\/([^?]+)(?:\?.*)?$/i);
            if (!match) { throw error; }
            return fetch('https://api.github.com/repos/' + match[1] + '/' +
                match[2] + '/contents/' + match[4] + '?ref=' + match[3], {
                cache: 'no-cache',
    ...
    function initMap(app) {
        ...
        function loadVectorOverlay(path) {
            ...
            return fetchText(versioned(path)).then(function (source) {
                ...
                showError('Contours géographiques indisponibles : ' + error.message);
            });
        }
        ...
        // Résoudre une publication immuable : le manifeste et les images
        // restent sur le même run, même si la branche data change entre-temps.
        Promise.resolve().then(function () {
            var match = baseUrl.match(/^https:\/\/raw\.githubusercontent\.com\/([^/]+)\/([^/]+)\/([^/]+)$/);
            if (!match || /^[a-f0-9]{40}$/i.test(match[3])) {
                return;
            }
            return fetchJson('https://api.github.com/repos/' + match[1] + '/' +
                match[2] + '/git/ref/heads/' + encodeURIComponent(match[3]))
                .then(function (ref) {
                    if (!ref.object || !/^[a-f0-9]{40}$/i.test(ref.object.sha)) {
                        throw new Error('Référence de publication GitHub invalide');
                    }
                    baseUrl = 'https://raw.githubusercontent.com/' + match[1] +
                        '/' + match[2] + '/' + ref.object.sha;
                })
                .catch(function () {
                    // API indisponible ou quota atteint : tenter la branche.
                    // Aucun jeton GitHub n'est exposé au navigateur.
                });
        }).then(function () {
            return fetchJson(baseUrl + '/maps/index.json');
        })
            .then(function (payload) {
            ...
            .catch(function (error) {
                showError('Impossible de charger les cartes GFS : ' + error.message);
            });
    }
