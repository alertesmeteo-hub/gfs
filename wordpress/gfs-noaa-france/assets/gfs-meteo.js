(function () {
    ...
    function temperatureClass(value) {
        ...
        if (value >= 35) { return 'gfs-temp-extreme'; }
        if (value >= 30) { return 'gfs-temp-hot'; }
        if (value >= 22) { return 'gfs-temp-warm'; }
        if (value >= 12) { return 'gfs-temp-mild'; }
        if (value >= 4) { return 'gfs-temp-cool'; }
        return 'gfs-temp-cold';
    }

    function gustClass(value) {
        if (!finite(value)) { return ''; }
        if (value >= 80) { return 'gfs-gust-danger'; }
        if (value >= 60) { return 'gfs-gust-strong'; }
        if (value >= 40) { return 'gfs-gust-moderate'; }
        return '';
    }

    function windForceClass(value) {
        if (!finite(value)) { return 'gfs-wind-calm'; }
        if (value >= 80) { return 'gfs-wind-violent'; }
        if (value >= 60) { return 'gfs-wind-strong'; }
        if (value >= 40) { return 'gfs-wind-fresh'; }
        if (value >= 20) { return 'gfs-wind-moderate'; }
        return 'gfs-wind-light';
    }

    function rainClass(value) {
        if (!finite(value) || value < 0.1) { return ''; }
        if (value >= 15) { return 'gfs-rain gfs-rain-intense'; }
        if (value >= 5) { return 'gfs-rain gfs-rain-strong'; }
        if (value >= 2) { return 'gfs-rain gfs-rain-moderate'; }
        if (value >= 0.5) { return 'gfs-rain gfs-rain-light'; }
        return 'gfs-rain gfs-rain-trace';
    }
    ...
    function chartDayBands(svg, count, margin, innerWidth, innerHeight, meta) {
        ...
        while (start < count) {
            ...
            svg.appendChild(svgElement('rect', {
                ...
                height: innerHeight,
                class: 'gfs-chart-day-band gfs-dayband-' + token
            }));
            ...
            svg.appendChild(svgElement('rect', {
                ...
                width: Math.max(0, endX - startX),
                height: 46,
                class: 'gfs-chart-day-strip gfs-daystrip-' + token
            }));
            ...
            if (start > 0) {
                svg.appendChild(svgElement('line', {
                    ...
                    class: 'gfs-chart-day-separator gfs-dayseparator-' + token
                }));
            }
            ...
            if (endX - startX > 72) {
                ...
                svg.appendChild(svgElement('text', {
                    ...
                    y: margin.top + 31,
                    'text-anchor': 'middle',
                    class: 'gfs-chart-day-label gfs-daylabel-' + token
                }, dayLabel));
            }
            start = end + 1;
        }
    ...
    function appendChartMetrics(container, metrics) {
        ...
        block.className = 'gfs-chart-metrics';
        valid.forEach(function (metric) {
            var item = document.createElement('span');
            item.className = 'gfs-chart-metric';
            item.appendChild(textSpan(metric.label, 'gfs-chart-metric-label'));
            item.appendChild(textSpan(metric.value, 'gfs-chart-metric-value'));
    ...
    function appendChartViewport(container, svg) {
        ...
        viewport.className = 'gfs-chart-viewport';
        var tooltip = document.createElement('div');
        tooltip.className = 'gfs-chart-tooltip';
        tooltip.hidden = true;
        ...
        function tooltipTarget(node) {
            ...
            return node.closest('[data-gfs-tooltip]');
        }
        ...
        viewport.addEventListener('pointerover', function (event) {
            ...
            if (!target) { return; }
            tooltip.textContent = target.getAttribute('data-gfs-tooltip') || '';
    ...
    function drawExtremaLabels(svg, points, values, formatter, className) {
        ...
        indexes.forEach(function (index) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: y,
                'text-anchor': 'middle',
                class: 'gfs-chart-value-label ' + className
    ...
    function renderLineChart(container, labels, series, options) {
        ...
        if (!scale) {
            container.appendChild(textSpan('Données indisponibles pour ce diagramme.', 'gfs-chart-empty'));
            return;
        }
        ...
        svg.classList.add('gfs-chart-svg');
        ...
        for (var g = 0; g <= tickValue; g += 1) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: gy,
                y2: gy,
                class: 'gfs-chart-grid'
            }));
            svg.appendChild(svgElement('text', {
                x: margin.left - 9,
                y: gy + 4,
                'text-anchor': 'end',
                class: 'gfs-chart-axis'
        ...
        chartTickIndexes(n, 8).forEach(function (index) {
            ...
            svg.appendChild(svgElement('line', {
                ...
                y1: margin.top,
                y2: margin.top + innerHeight,
                class: 'gfs-chart-grid gfs-chart-grid-vertical'
            }));
            svg.appendChild(svgElement('text', {
                x: x,
                y: height - 18,
                'text-anchor': index === 0 ? 'start' : (index === n - 1 ? 'end' : 'middle'),
                class: 'gfs-chart-axis gfs-chart-xaxis'
            }, axisLabel));
        });
        ...
        series.forEach(function (item, seriesIndex) {
            ...
            if (series.length === 1 && options.fillArea) {
                segments.forEach(function (segment) {
                    ...
                    svg.appendChild(svgElement('path', {
                        d: d,
                        class: 'gfs-chart-area ' + item.className
                    }));
                });
            }
            ...
            segments.forEach(function (segment) {
                svg.appendChild(svgElement('path', {
                    ...
                    class: 'gfs-chart-line ' + item.className,
                    fill: 'none'
                }));
            });
            ...
            points.forEach(function (point, index) {
                ...
                var attrs = {
                    ...
                    class: (options.showPoints === false ? 'gfs-chart-hitpoint ' : 'gfs-chart-point ') + item.className,
                    tabindex: 0,
                    'aria-label': tooltipLabel,
                    'data-gfs-tooltip': tooltipLabel
                };
        ...
        if (options.bandBetween && seriesPoints.length >= 2) {
            ...
            if (upperPoints.length > 1 && upperPoints.length === lowerPoints.length) {
                ...
                var envelope = svgElement('path', {
                    d: area,
                    class: 'gfs-chart-wind-envelope'
                });
                var firstLine = svg.querySelector('.gfs-chart-line');
        ...
        if (series.length > 1) {
            var legend = svgElement('g', { class: 'gfs-chart-legend' });
            series.forEach(function (item, index) {
                ...
                legend.appendChild(svgElement('text', {
                    x: x + 36,
                    y: 21,
                    class: 'gfs-chart-axis gfs-chart-legend-text'
                }, item.label));
            });
    ...
    function rainVisualClass(value) {
        if (!finite(value) || value < 0.1) { return 'gfs-chart-rain-zero'; }
        if (value >= 15) { return 'gfs-chart-rain-intense'; }
        if (value >= 5) { return 'gfs-chart-rain-strong'; }
        if (value >= 2) { return 'gfs-chart-rain-moderate'; }
        if (value >= 0.5) { return 'gfs-chart-rain-light'; }
        return 'gfs-chart-rain-trace';
    }
    ...
    function renderRainChart(container, labels, rainValues, cumulativeValues, options) {
        ...
        for (var cg = 0; cg <= cumTickCount; cg += 1) {
            ...
            svg.appendChild(svgElement('text', {
                ...
                y: cy + 5,
                'text-anchor': 'start',
                class: 'gfs-chart-axis gfs-chart-axis-cumulative'
        ...
        rainValues.forEach(function (value, index) {
            ...
            var bar = svgElement('rect', {
                ...
                class: 'gfs-chart-rain-bar ' + rainVisualClass(value),
                tabindex: 0,
                'aria-label': rainTooltip,
                'data-gfs-tooltip': rainTooltip
            });
            svg.appendChild(bar);
        });
        ...
        if (cleanPoints.length) {
            svg.appendChild(svgElement('path', {
                ...
                class: 'gfs-chart-line gfs-series-rain-cumulative',
                fill: 'none'
            }));
            cumulativePoints.forEach(function (point, index) {
                ...
                var hitPoint = svgElement('circle', {
                    ...
                    cy: point.y,
                    r: 11,
                    class: 'gfs-chart-hitpoint gfs-series-rain-cumulative',
                    tabindex: 0,
                    'aria-label': cumulativeTooltip,
                    'data-gfs-tooltip': cumulativeTooltip,
                    fill: 'transparent',
                    stroke: 'none',
                    opacity: '0'
                });
        ...
        legend.appendChild(svgElement('rect', {
            ...
            height: 11,
            rx: 4,
            class: 'gfs-chart-rain-bar gfs-chart-rain-light'
        }));
        legend.appendChild(svgElement('text', {
            x: margin.left + 28,
            y: 21,
    ...
    function initApp(app) {
        ...
        var titlePrefix = app.dataset.titlePrefix || 'Prévisions GFS';

        var input = app.querySelector('.gfs-city-input');
        var locateButton = app.querySelector('[data-gfs-locate]');
        var results = app.querySelector('.gfs-search-results');
        var status = app.querySelector('.gfs-search-status');
        var generalBody = app.querySelector('[data-gfs-body-general]');
        var stormBody = app.querySelector('[data-gfs-body-storms]');
        var stormSummary = app.querySelector('[data-gfs-storm-summary]');
        var snowBody = app.querySelector('[data-gfs-body-snow]');
        var snowSummary = app.querySelector('[data-gfs-snow-summary]');
        var tabs = Array.prototype.slice.call(app.querySelectorAll('[data-gfs-tab]'));
        var panels = Array.prototype.slice.call(app.querySelectorAll('[data-gfs-panel]'));
        var title = app.querySelector('[data-gfs-title]');
        var altitudeLine = app.querySelector('[data-gfs-altitude]');
        var meta = app.querySelector('[data-gfs-meta]');
        var generated = app.querySelector('[data-gfs-generated]');
        var stale = app.querySelector('[data-gfs-stale]');
        var stormTopScroll = app.querySelector('[data-gfs-top-scroll="storms"]');
        var stormScrollWrap = app.querySelector('[data-gfs-scroll-wrap="storms"]');
        var stormTable = app.querySelector('.gfs-storm-table');
        var snowTopScroll = app.querySelector('[data-gfs-top-scroll="snow"]');
        var snowScrollWrap = app.querySelector('[data-gfs-scroll-wrap="snow"]');
        var snowTable = app.querySelector('.gfs-snow-table');
        var chartTemperature = app.querySelector('[data-gfs-chart-temperature]');
        var chartPressure = app.querySelector('[data-gfs-chart-pressure]');
        var chartRain = app.querySelector('[data-gfs-chart-rain]');
        var chartWind = app.querySelector('[data-gfs-chart-wind]');
        var chartTitleTemperature = app.querySelector('[data-gfs-chart-title-temperature]');
        var chartTitlePressure = app.querySelector('[data-gfs-chart-title-pressure]');
        var chartTitleRain = app.querySelector('[data-gfs-chart-title-rain]');
        var chartTitleWind = app.querySelector('[data-gfs-chart-title-wind]');
        var rainTotal = app.querySelector('[data-gfs-rain-total]');
        var fullscreenButton = app.querySelector('[data-gfs-fullscreen]');
        ...
        function makeDayCell(date, rowSpan) {
            var cell = createCell('th', 'Date', 'gfs-day-cell gfs-day-' + weekdayToken(date));
            cell.scope = 'rowgroup';
            cell.rowSpan = rowSpan;
            cell.appendChild(textSpan(weekdayFormat.format(date), 'gfs-day-weekday'));
            cell.appendChild(textSpan(shortDateFormat.format(date), 'gfs-day-date'));
            return cell;
        }
        ...
        function setStatus(message, error) {
            ...
            status.classList.toggle('gfs-search-error', Boolean(error));
        }

        function setActiveView(view) {
            tabs.forEach(function (button) {
                var active = button.dataset.gfsTab === view;
                button.classList.toggle('is-active', active);
                button.setAttribute('aria-selected', active ? 'true' : 'false');
            });
            panels.forEach(function (panel) {
                panel.hidden = panel.dataset.gfsPanel !== view;
            });
            ...
            if (view === 'map' && selectedMapFocus) {
                window.requestAnimationFrame(function () {
                    var mapApp = app.querySelector('[data-gfsm-app]');
                    if (mapApp) {
                        mapApp.dispatchEvent(new CustomEvent('gfsm:focus-location', {
        ...
        tabs.forEach(function (button) {
            button.addEventListener('click', function () {
                setActiveView(button.dataset.gfsTab || 'general');
            });
        });
        ...
        function putMessage(body, message, error, colspan) {
            ...
            var cell = createCell('td', '', error ? 'gfs-loading gfs-load-error' : 'gfs-loading');
        ...
        function showTableMessage(message, error) {
            ...
            if (stormSummary) {
                ...
                stormSummary.classList.toggle('gfs-storm-summary-warning', Boolean(error));
            }
            if (snowSummary) {
                snowSummary.textContent = message;
                snowSummary.classList.toggle('gfs-snow-summary-warning', Boolean(error));
            }
        }
            ...
            return fetchJson(baseUrl + '/index.json', { cache: 'no-cache' })
                .then(function (payload) {
                    if (!payload || payload.status !== 'ok' || !payload.departments) {
                        throw new Error('Index national GFS invalide');
                    }
                    indexData = payload;
                    ...
                    meta.textContent = 'Run du ' + run + ' • résolution ' + resolution +
                        ((payload.model && payload.model.storm_diagnostics) ? ' • diagnostics orages + neige GFS' : '');
        ...
        function loadDepartment(code) {
            ...
            if (!indexData || !indexData.departments[normalized]) {
                return Promise.reject(new Error('Département hors couverture GFS'));
            }
        ...
        function renderForecast(departmentData, commune) {
            ...
            function appendHazard(row, label, code) {
                ...
                var cell = createCell('td', label, numeric === null ? '' : 'gfs-hazard gfs-hazard-' + numeric);
            ...
            if (!forecasts.length) {
                showTableMessage('Aucune échéance GFS disponible pour cette commune.', true);
                return;
            }
            ...
            forecasts.forEach(function (step, currentIndex) {
                ...
                if (dayKey !== previousGeneralDay) {
                    row.classList.add('gfs-new-day');
                    row.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousGeneralDay = dayKey;
                }

                var hourCell = createCell('td', 'Heure', 'gfs-hour gfs-hour-' + weekdayToken(date));
                ...
                var conditionCell = createCell('td', 'Temps', 'gfs-condition');
                conditionCell.appendChild(textSpan(condition.icon, 'gfs-icon'));
                conditionCell.appendChild(textSpan(condition.label));
                row.appendChild(conditionCell);

                var temperatureCell = createCell('td', 'Température', 'gfs-temperature ' + temperatureClass(temp));
                ...
                var windCell = createCell('td', 'Vent', 'gfs-wind-cell ' + windForceClass(windDisplay));
                ...
                if (dayKey !== previousStormDay) {
                    stormRow.classList.add('gfs-new-day');
                    stormRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousStormDay = dayKey;
                }
                var stormHour = createCell('td', 'Heure', 'gfs-hour gfs-hour-' + weekdayToken(date));
                ...
                var thunderCell = createCell('td', 'Risque orage', thunder ? 'gfs-thunder-risk gfs-thunder-' + Number(thunderCode) : '');
                if (thunder) {
                    thunderCell.appendChild(textSpan(thunder.icon + ' ', 'gfs-risk-icon'));
                    thunderCell.appendChild(textSpan(thunder.label, 'gfs-risk-label'));
                } else {
                ...
                var capeCell = createCell('td', 'CAPE', finite(cape) && cape >= 1500 ? 'gfs-cape-high' : (finite(cape) && cape >= 500 ? 'gfs-cape-mid' : ''));
                capeCell.textContent = finite(cape) && Number(cape) >= 25 ? formatNumber(cape, 0) + ' J/kg' : '—';
                capeCell.title = 'MUCAPE instantanée directement produite par GFS ; les valeurs quasi nulles ne sont pas affichées.';
                ...
                appendNumber(stormRow, 'Foudre', lightning, 0, '/100', finite(lightning) && lightning >= 60 ? 'gfs-lightning-high' : '');
                ...
                var stormTypeCell = createCell('td', 'Type d’orage', 'gfs-storm-type');
                ...
                var detailsCell = createCell('td', 'Détails', 'gfs-details-cell');
                var detailsButton = document.createElement('button');
                detailsButton.type = 'button';
                detailsButton.className = 'gfs-details-button';
                detailsButton.textContent = 'Afficher';
                detailsButton.setAttribute('aria-expanded', 'false');
                detailsCell.appendChild(detailsButton);

                var advanced = document.createElement('div');
                advanced.className = 'gfs-advanced-list gfs-details-inline';
                ...
                [
                    ...
                    box.className = 'gfs-advanced-item';
                    box.appendChild(textSpan(item[0], 'gfs-advanced-label'));
                    box.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'gfs-advanced-value'));
                ...
                if (dayKey !== previousSnowDay) {
                    snowRow.classList.add('gfs-new-day');
                    snowRow.appendChild(makeDayCell(date, dayCounts[dayKey]));
                    previousSnowDay = dayKey;
                }
                var snowHour = createCell('td', 'Heure', 'gfs-hour gfs-hour-' + weekdayToken(date));
                ...
                var snowRiskCell = createCell('td', 'Risque neige', snowRisk ? 'gfs-snow-risk gfs-snow-' + Number(snowRiskCode) : '');
                if (snowRisk) {
                    snowRiskCell.appendChild(textSpan(snowRisk.icon + ' ', 'gfs-risk-icon'));
                    snowRiskCell.appendChild(textSpan(snowRisk.label, 'gfs-risk-label'));
                ...
                var stickCell = createCell('td', 'Tenue', finite(stickCode) ? 'gfs-stick gfs-stick-' + Number(stickCode) : '');
                ...
                var snowWindCell = createCell('td', 'Vent moyen / rafales', 'gfs-snow-wind ' + windForceClass(gustDisplay));
                ...
                var snowDetailsCell = createCell('td', 'Détails', 'gfs-details-cell gfs-snow-details-cell');
                var snowDetailsButton = document.createElement('button');
                snowDetailsButton.type = 'button';
                snowDetailsButton.className = 'gfs-details-button';
                ...
                snowAdvanced.className = 'gfs-snow-profile gfs-details-inline';
                snowAdvanced.hidden = true;

                function appendProfileSection(titleText, items) {
                    var section = document.createElement('section');
                    section.className = 'gfs-profile-section';
                    section.appendChild(textSpan(titleText, 'gfs-profile-title'));
                    var list = document.createElement('div');
                    list.className = 'gfs-profile-list';
                    items.forEach(function (item) {
                        var profileItem = document.createElement('div');
                        profileItem.className = 'gfs-profile-item';
                        profileItem.appendChild(textSpan(item[0], 'gfs-profile-label'));
                        profileItem.appendChild(textSpan(finite(item[1]) ? formatNumber(item[1], item[2]) + item[3] : '—', 'gfs-profile-value'));
                ...
                appendProfileSection('Sorties et diagnostics GFS', [
            ...
            if (stormSummary) {
                stormSummary.classList.toggle('gfs-storm-summary-warning', !hasStormColumns);
                if (!hasStormColumns) {
            ...
            if (snowSummary) {
                snowSummary.classList.toggle('gfs-snow-summary-warning', !hasSnowColumns);
                if (!hasSnowColumns) {
                    ...
                    snowSummary.textContent = 'Risque de neige maximal sur les ' + forecasts.length + ' prochaines échéances : ' + maxSnow.icon + ' ' + maxSnow.label + ' • données neige GFS.';
                }
            }
            ...
            if (altitudeLine) {
                altitudeLine.textContent = finite(altitude)
                    ? 'Altitude de ' + cityName + ' : ≈ ' + formatNumber(altitude, 0) + ' m (point de grille GFS)'
                    : 'Altitude de ' + cityName + ' : — (relancez le workflow GitHub GFS)';
                altitudeLine.classList.toggle('gfs-altitude-missing', !finite(altitude));
            }
            selectedMapFocus = {
                latitude: Number(commune[4]),
                longitude: Number(commune[5]),
                label: cityName + (postal ? ' (' + postal + ')' : ''),
                // Le GFS 0,25° a une maille d'environ 28 km : un zoom x6
            ...
            renderLineChart(chartTemperature, chartLabels, [{ label: 'Température', values: chartTemps, className: 'gfs-series-temperature' }], {
                decimals: 0,
                unit: '°C',
                fillArea: false,
            ...
            renderLineChart(chartPressure, chartLabels, [{ label: 'Pression', values: chartPressures, className: 'gfs-series-pressure' }], {
                decimals: 0,
                unit: 'hPa',
                fillArea: false,
                metricMode: 'pressure',
            ...
            renderLineChart(chartWind, chartLabels, [
                { label: 'Vent moyen', values: chartWindValues, className: 'gfs-series-wind' },
                { label: 'Rafales', values: chartGustValues, className: 'gfs-series-gust' }
            ], {
                decimals: 0,
                unit: 'km/h',
                forceZero: true,
        ...
        function selectCommune(candidate) {
            ...
            loadDepartment(candidate.codeDepartement)
                .then(function (departmentData) {
                    ...
                    if (!commune) {
                        throw new Error('Commune absente du catalogue GFS');
                    }
        ...
        function displaySearchResults(candidates) {
            ...
            candidates.forEach(function (candidate, position) {
                ...
                button.type = 'button';
                button.className = 'gfs-search-result';
                button.id = input.id + '-option-' + position;
                button.setAttribute('role', 'option');
                button.setAttribute('aria-selected', 'false');

                var name = textSpan(candidate.nom, 'gfs-result-name');
        ...
        if (window.GFS_AUTOHEAL && window.GFS_AUTOHEAL.url) {
            fetch(window.GFS_AUTOHEAL.url, { method: 'POST', cache: 'no-store' }).catch(function () {});
        }

        if (!baseUrl) {
            showTableMessage('Adresse des données GFS non configurée.', true);
            return;
        }
        loadIndex()
            .then(function () {
    ...
    whenReady(function () {
        document.querySelectorAll('[data-gfs-app]').forEach(initApp);
    });
}());
