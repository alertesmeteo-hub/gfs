...
define('GFS_VERSION', '1.1.4');
define('GFS_RELEASE_DATE', '21/09/2026');
...
// Auto-guérison du pipeline GFS : si index.json est resté bloqué trop
...
//   define('GFS_GITHUB_TOKEN', 'github_pat_xxx...');
// Jeton « fine-grained », limité au dépôt alertesmeteo-hub/gfs, permission
// « Actions » en Read and write.
define('GFS_GITHUB_REPO', 'alertesmeteo-hub/gfs');
define('GFS_GITHUB_DATA_BRANCH', 'data');
define('GFS_GITHUB_WORKFLOW_BRANCH', 'main');
define('GFS_GITHUB_WORKFLOW_FILE', 'update-gfs.yml');
// Quatre runs par jour (00/06/12/18 UTC) : seuil resserré en conséquence.
define('GFS_STALE_THRESHOLD_MIN', 8 * 60);
...
add_action('wp_ajax_gfs_autoheal', 'gfs_handle_autoheal');
add_action('wp_ajax_nopriv_gfs_autoheal', 'gfs_handle_autoheal');

function gfs_handle_autoheal() {
    if (!defined('GFS_GITHUB_TOKEN') || !GFS_GITHUB_TOKEN) {
        wp_send_json_success(array('configured' => false));
    }

    if (get_transient('gfs_autoheal_lock')) {
        wp_send_json_success(array('skipped' => true));
    }
    set_transient('gfs_autoheal_lock', 1, 5 * MINUTE_IN_SECONDS);

    $generated_at = gfs_fetch_generated_at();
    if (null === $generated_at) {
        wp_send_json_success(array('configured' => true, 'checked' => false));
    }

    $age_minutes = (time() - $generated_at) / 60;
    if ($age_minutes <= GFS_STALE_THRESHOLD_MIN) {
        wp_send_json_success(array('configured' => true, 'stale' => false, 'age_minutes' => round($age_minutes)));
    }

    if (get_transient('gfs_autoheal_cooldown')) {
        wp_send_json_success(array('configured' => true, 'stale' => true, 'triggered' => false, 'cooldown' => true));
    }
    set_transient('gfs_autoheal_cooldown', 1, 30 * MINUTE_IN_SECONDS);

    $triggered = gfs_trigger_workflow();
    wp_send_json_success(array('configured' => true, 'stale' => true, 'triggered' => $triggered));
}

function gfs_fetch_generated_at() {
    $url = 'https://api.github.com/repos/' . GFS_GITHUB_REPO . '/contents/index.json'
        . '?ref=' . rawurlencode(GFS_GITHUB_DATA_BRANCH);
    $response = wp_remote_get($url, array(
        'headers' => array(
            'Accept'     => 'application/vnd.github.raw',
            'User-Agent' => 'gfs-noaa-france-autoheal',
        ),
        'timeout' => 8,
    ));
...
function gfs_trigger_workflow() {
    $url = 'https://api.github.com/repos/' . GFS_GITHUB_REPO . '/actions/workflows/'
        . rawurlencode(GFS_GITHUB_WORKFLOW_FILE) . '/dispatches';
    $response = wp_remote_post($url, array(
        'headers' => array(
            'Accept'        => 'application/vnd.github+json',
            'Authorization' => 'Bearer ' . GFS_GITHUB_TOKEN,
            'Content-Type'  => 'application/json',
            'User-Agent'    => 'gfs-noaa-france-autoheal',
        ),
        'body'    => wp_json_encode(array('ref' => GFS_GITHUB_WORKFLOW_BRANCH)),
        'timeout' => 8,
    ));
...
function gfs_register_assets() {
    ...
    wp_localize_script('gfs-table', 'GFS_AUTOHEAL', array(
        'url' => admin_url('admin-ajax.php?action=gfs_autoheal'),
    ));
}
...
function gfs_render_settings_page() {
    ...
    <div class="wrap">
        ...
        <p>
            Statut : <strong><?php echo (defined('GFS_GITHUB_TOKEN') && GFS_GITHUB_TOKEN) ? '✅ Configurée' : '⚠️ Non configurée'; ?></strong>
        </p>
        <p>
            Si <code>index.json</code> reste bloqué plus de <?php echo esc_html((int) round(GFS_STALE_THRESHOLD_MIN / 60)); ?> heures,
            chaque chargement de cette page relance automatiquement le pipeline sur GitHub. Pour l'activer, ajouter dans
            <code>wp-config.php</code> :
        </p>
        <p><code>define('GFS_GITHUB_TOKEN', 'github_pat_xxx...');</code></p>
        <p>
            Jeton « fine-grained » GitHub, limité au dépôt <code>alertesmeteo-hub/gfs</code>, permission
    ...
    >
        <header class="gfs-header">
            ...
            <div class="gfs-header-actions">
                ...
                <button
                    type="button"
                    class="gfs-fullscreen-toggle"
                    data-gfs-fullscreen
                    title="Plein écran"
                    aria-label="Afficher le module en plein écran"
                >⛶</button>
            </div>
        </header>
